...
export function NavBar() {
  ...
  const linkClass =
    "rounded-full px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] text-muted-foreground transition hover:bg-secondary/60 hover:text-foreground";

  return (
    <header className="sticky top-0 z-40 border-b border-border/60 bg-background/85 backdrop-blur">
      <nav className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-2 px-4 py-3">
        <Link to="/" className="font-script text-2xl leading-none text-foreground">
          Azurie Sage
        </Link>
